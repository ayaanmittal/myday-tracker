-- Corrected Leave Deduction Calculation
...
CREATE OR REPLACE FUNCTION public.calculate_employee_leave_deductions_corrected(
  p_user_id UUID,
...
BEGIN
  ...
  -- Get all unpaid leave records for this employee in this month
  ...
  LOOP
    ...
    -- Check if this date is an office holiday (from company_holidays table)
    SELECT EXISTS(
...
GRANT EXECUTE ON FUNCTION public.calculate_employee_leave_deductions_corrected(UUID, DATE, NUMERIC) TO authenticated;

-- Update the existing function to use the corrected logic
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM public.calculate_employee_leave_deductions_corrected(
    p_user_id,
    p_payment_month,
    p_deduction_percentage
  );
