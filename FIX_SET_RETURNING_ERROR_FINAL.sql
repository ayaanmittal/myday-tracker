-- Fix the set-returning function error in the ultimate fix
-- The issue is with generate_series in WHERE clause - need to use proper subquery

-- 1. Drop and recreate the function with proper Sunday counting
...
BEGIN
  ...
  -- Count Sundays in the month (fixed approach)
  SELECT COUNT(*)
  INTO v_sundays
  FROM (
...
-- 4. Test the function
SELECT 
  'Testing fixed function' as test_name,
  total_deductions,
  total_paid_leaves,
  total_unpaid_leaves,
...
-- 5. Also fix the test function
DROP FUNCTION IF EXISTS test_office_holidays_count(DATE);
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_company_holidays
  ...
  v_count := v_company_holidays + v_sundays;
  RETURN v_count;
END;
$$;
