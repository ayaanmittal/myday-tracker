-- Create API refresh logs table
-- Run this in your Supabase SQL editor

CREATE TABLE IF NOT EXISTS public.api_refresh_logs (
...
-- Create policies
CREATE POLICY IF NOT EXISTS "Admins can view all API refresh logs"
  ON public.api_refresh_logs FOR SELECT
  USING (has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Admins can insert API refresh logs"
  ON public.api_refresh_logs FOR INSERT
  WITH CHECK (has_role(auth.uid(), 'admin'));

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_api_refresh_logs_created_at ON public.api_refresh_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_api_refresh_logs_admin_user_id ON public.api_refresh_logs(admin_user_id);
