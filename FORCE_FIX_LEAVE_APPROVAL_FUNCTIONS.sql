-- FORCE FIX Leave Approval Functions
-- This script aggressively drops and recreates the functions to clear any cache

-- Step 1: Drop ALL existing functions with any parameter variations
...
DROP FUNCTION IF EXISTS public.reject_leave_request(UUID, UUID);
DROP FUNCTION IF EXISTS public.reject_leave_request(UUID, UUID, TEXT);

-- Step 2: Clear any cached function definitions
-- This forces PostgreSQL to reload function definitions
SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname = 'approve_leave_request';
SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname = 'reject_leave_request';

-- Step 3: Create the approve_leave_request function with EXPLICIT column references
...
BEGIN
  ...
  -- Get the leave request details with EXPLICIT column selection
  SELECT 
    lr.id,
    lr.user_id,
    lr.leave_type_id,
...
-- Step 4: Create the reject_leave_request function
...
-- Step 6: Verify functions were created and show their definitions
...
-- Step 7: Test the function definition to ensure it's correct
