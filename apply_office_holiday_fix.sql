-- Apply office holiday fix directly to database
-- This script fixes the mark_users_holiday_range function and creates the new mark_office_holiday_range function

-- First, update the manual_status constraint to allow 'Office Holiday'
...
-- Fix mark_users_holiday_range function to use profiles.is_active instead of profiles.active
...
begin
  ...
  if user_ids is null or array_length(user_ids, 1) is null or array_length(user_ids, 1) = 0 then
    ...
    where coalesce(p.is_active, true) = true; -- Fixed: use p.is_active instead of p.active
  else
    v_user_ids := user_ids;
  end if;
...
-- Create a new function specifically for office holidays
