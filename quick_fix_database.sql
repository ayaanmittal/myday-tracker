-- Quick Fix for "No work history available" issue
...
BEGIN
  ...
  IF user_count > 0 AND entry_count = 0 THEN
    -- Create sample entries for the first user
    ...
    SELECT 
      u.id,
      CURRENT_DATE - (s.i)::INTEGER,
      (CURRENT_DATE - (s.i)::INTEGER) + INTERVAL '9 hours',
      (CURRENT_DATE - (s.i)::INTEGER) + INTERVAL '17 hours',
      480, -- 8 hours in minutes
      'completed'
    FROM auth.users u
    CROSS JOIN generate_series(0, 6) AS s(i)
    WHERE u.id = (SELECT id FROM auth.users LIMIT 1)
    AND NOT EXISTS (
      SELECT 1 FROM public.day_entries de2 
      WHERE de2.user_id = u.id AND de2.entry_date = CURRENT_DATE - (s.i)::INTEGER
    );
    
    -- Create sample day updates
    ...
    SELECT 
      ...
      CASE WHEN s.i = 2 THEN 'Network issues' ELSE NULL END
    FROM public.day_entries de
    CROSS JOIN generate_series(0, 6) AS s(i)
    WHERE de.entry_date = CURRENT_DATE - (s.i)::INTEGER
    AND NOT EXISTS (
      SELECT 1 FROM public.day_updates du2 
      WHERE du2.day_entry_id = de.id
    );
