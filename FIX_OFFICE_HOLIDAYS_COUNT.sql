...
-- The issue is that we're overwriting the company holidays count instead of adding to it

-- 1. Drop and recreate the get_employee_salary_summary function with fixed logic
...
SELECT 
  'Testing office holidays count' as test_name,
  total_deductions,
  total_paid_leaves,
  total_unpaid_leaves,
...
-- 5. Let's also verify the individual counts
SELECT 
