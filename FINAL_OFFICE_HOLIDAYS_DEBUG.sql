-- Debug and fix the office holidays counting issue
-- The function should count both company holidays AND Sundays

-- 1. First, let's check what's actually in the company_holidays table
SELECT 
  'Current company holidays in October 2025' as test_name,
  holiday_date,
  title,
...
-- 2. Count company holidays manually
SELECT 
  'Manual count of company holidays' as test_name,
  COUNT(*) as count
FROM public.company_holidays 
WHERE holiday_date >= '2025-10-01' AND holiday_date <= '2025-10-31';

-- 3. Count Sundays manually
SELECT 
  'Manual count of Sundays' as test_name,
  COUNT(*) as count
FROM (
...
-- 4. If company holidays are missing, insert them
...
-- 5. Drop and recreate the function with explicit debugging
...
BEGIN
  ...
  -- STEP 1: Count company holidays in the month
  SELECT COUNT(*)
  ...
  -- STEP 2: Count Sundays in the month
  SELECT COUNT(*)
  INTO v_sundays
  FROM (
  ...
  RAISE NOTICE 'DEBUG: Month=%, Start=%, End=%', p_month, v_month_start, v_month_end;
  RAISE NOTICE 'DEBUG: Company holidays=%', v_company_holidays;
  RAISE NOTICE 'DEBUG: Sundays=%', v_sundays;
  RAISE NOTICE 'DEBUG: Total office holidays=%', v_office_holidays;
...
-- 7. Test the function
SELECT 
  'Testing RPC function with explicit debugging' as test_name,
  total_deductions,
  total_paid_leaves,
  total_unpaid_leaves,
...
SELECT 
  'Final verification: Expected total should be 6' as test_name,
  (SELECT COUNT(*) FROM public.company_holidays WHERE holiday_date >= '2025-10-01' AND holiday_date <= '2025-10-31') as company_holidays,
