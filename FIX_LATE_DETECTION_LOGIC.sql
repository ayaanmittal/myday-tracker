-- Fix Late Detection Logic
-- This script centralizes late detection using the existing database function
-- and ensures all processing methods use consistent logic

-- Step 1: Update the existing is_checkin_late function to read from settings by default
...
-- Step 3: Update the process_teamoffice_attendance function to use proper late detection
...
-- Step 4: Create a function to update late status for existing records
...
BEGIN
  ...
  SET is_late = public.is_checkin_late(check_in_at),
      updated_at = NOW()
