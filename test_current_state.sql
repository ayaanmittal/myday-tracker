...
-- This will help us understand what's happening with the leaves system

-- Step 1: Check if leaves table exists
SELECT 'Step 1: Check if leaves table exists' as step;
SELECT 
  table_name,
  column_name,
  data_type
...
-- Step 2: Check leave requests status
SELECT 'Step 2: Check leave requests status' as step;
SELECT 
  lr.status,
  lr.processed,
  COUNT(*) as count
...
-- Step 3: Check approved leave requests that need processing
SELECT 'Step 3: Check approved leave requests that need processing' as step;
SELECT 
  p.name,
  lr.start_date,
  lr.end_date,
...
-- Step 4: Check employee categories and leave policies
SELECT 'Step 4: Check employee categories and leave policies' as step;
SELECT 
...
-- Step 5: Check leave policies for intern category
SELECT 'Step 5: Check leave policies for intern category' as step;
SELECT 
  ec.name as category_name,
  lt.name as leave_type_name,
  lp.is_paid,
...
-- Step 6: Check current leaves table content
SELECT 'Step 6: Check current leaves table content' as step;
...
-- Step 8: Test the populate function if it exists
SELECT 'Step 8: Test populate function' as step;
SELECT public.populate_leaves_from_requests() as leaves_created;

-- Step 9: Check leaves table after processing
SELECT 'Step 9: Check leaves table after processing' as step;
SELECT 
  p.name,
  l.leave_date,
  l.leave_type_name,
...
-- Step 10: Check unpaid leave calculation after processing
SELECT 'Step 10: Check unpaid leave calculation after processing' as step;
SELECT 
  p.name,
