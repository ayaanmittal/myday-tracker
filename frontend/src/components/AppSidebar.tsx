...
export function AppSidebar() {
  ...
  return (
    <Sidebar className={`${collapsed ? 'w-16' : 'w-64'} bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 border-r border-gray-700 shadow-sm`} collapsible="icon" side="left">
      <SidebarContent className="bg-transparent">
        {/* Logo Section */}
        <div className={`${collapsed ? 'p-4 flex justify-center items-center' : 'p-6 flex justify-center items-center'}`}>
          <img 
            src="/logo.png" 
            alt="ERCMAX Logo" 
            className={`${collapsed ? 'max-h-10 max-w-full w-auto h-auto' : 'h-20 w-auto'} object-contain`}
          />
        </div>

        <SidebarGroup className={collapsed ? 'mt-2' : ''}>
          <SidebarGroupLabel className="text-gray-400 font-medium px-4 text-xs uppercase tracking-wider">
            {collapsed ? '' : 'Menu'}
          </SidebarGroupLabel>
          <SidebarGroupContent>
                          ...
                          {!collapsed && (
                            <div className="flex items-center justify-between flex-1 min-w-0">
                              ...
                              {item.badge && item.badge > 0 && (
                                <Badge className="ml-2 bg-red-600 text-white text-xs font-medium px-1.5 py-0.5 rounded-full min-w-[1.25rem] h-5 flex items-center justify-center">
                                  {item.badge}
                                </Badge>
                              )}
                            </div>
        ...
        <div className="mt-auto px-4 pb-4 border-t border-gray-700 pt-4 space-y-3">
          {!collapsed && userProfile && (
            <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-red-600 to-red-700 rounded-xl border border-red-500/30">
              <div className="h-10 w-10 rounded-full bg-gradient-to-br from-red-600 to-red-700 flex items-center justify-center text-white font-semibold shadow-sm border-2 border-white/20">
                {userProfile?.name ? userProfile.name.charAt(0).toUpperCase() : 'U'}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-white truncate">{userProfile?.name || 'User'}</div>
                <div className="text-xs text-gray-300 truncate">{userProfile?.email || 'email@example.com'}</div>
                {role && (
                  <div className="text-xs text-gray-400 mt-0.5 capitalize">{role}</div>
                )}
              </div>
            </div>
          )}
          ...
          <button
            onClick={signOut}
            className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gray-700/50 transition-all duration-200 w-full text-gray-300 font-medium border border-gray-700 hover:border-gray-600 bg-gray-800/50 hover:shadow-sm"
          >
