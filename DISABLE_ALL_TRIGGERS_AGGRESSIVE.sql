-- Disable All Triggers Aggressively
-- This script disables all triggers and tests if that stops the auto-marking

-- Step 1: List all triggers on unified_attendance
SELECT 'Step 1: List all triggers on unified_attendance' as step;
SELECT 
  trigger_name,
  event_manipulation,
  action_timing,
...
-- Step 2: Disable ALL triggers on unified_attendance
SELECT 'Step 2: Disable all triggers' as step;
DO $$
DECLARE
...
-- Step 3: Test our function with all triggers disabled
SELECT 'Step 3: Test function with all triggers disabled' as step;
...
WHERE ua.entry_date = '2025-02-08'::DATE
ORDER BY p.name;

-- Step 5: If it worked, we need to find the specific trigger and modify it
-- Let's check what triggers we disabled
SELECT 'Step 5: Check disabled triggers' as step;
SELECT 
  trigger_name,
  action_statement
FROM information_schema.triggers 
...
FROM public.unified_attendance ua
JOIN public.profiles p ON p.id = ua.user_id
WHERE ua.entry_date = '2025-02-09'::DATE
ORDER BY p.name;
