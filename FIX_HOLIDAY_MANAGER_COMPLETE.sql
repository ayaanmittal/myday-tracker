-- Complete fix for holiday manager to insert into company_holidays and update unified_attendance
-- This function will:
-- 1. Insert office holidays into company_holidays table
-- 2. Update unified_attendance with correct status and manual_status
-- 3. Handle both individual dates and date ranges
...
BEGIN
  ...
  -- Insert one record per date in the range
  ...
  SELECT u_ids.u_id AS user_id,
         ...
         'manual' AS source,
         'holiday' AS status,  -- status = 'holiday' for office holidays
         'Office Holiday'::VARCHAR AS manual_status,  -- manual_status = 'Office Holiday'
  ...
  SET manual_status = 'Office Holiday',
      status = 'holiday',  -- status = 'holiday' for office holidays
...
-- Update the existing function to use the complete version
...
-- Create a function to mark single date as office holiday
