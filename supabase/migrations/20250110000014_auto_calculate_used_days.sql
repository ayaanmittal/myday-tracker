-- Update refresh_employee_leave_balances function to automatically calculate used days from leave_requests
...
DECLARE
    ...
    actual_used_days INTEGER;
    actual_probation_used_days INTEGER;
BEGIN
        ...
        BEGIN
            ...
            IF employee_record.is_paid_leave_eligible THEN
                ...
                LOOP
                    ...
                    SELECT COALESCE(SUM(days_requested), 0) INTO actual_used_days
                    ...
                    IF is_on_probation THEN
                        used_days := 0;
                        probation_used_days := actual_used_days;
                    ELSE
                        used_days := actual_used_days;
                        probation_used_days := 0;
                    END IF;
                    
                    -- Insert the balance record (remaining_days will be calculated automatically)
    ...
    SELECT json_build_object(
        ...
        'message', 'Leave balances refreshed successfully with automatic used days calculation'
    ) INTO result;
