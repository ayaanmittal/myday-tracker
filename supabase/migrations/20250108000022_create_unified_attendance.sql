-- Create unified attendance table from scratch
-- This will be the single source of truth for all attendance data
...
CREATE TABLE public.unified_attendance (
  ...
  status TEXT NOT NULL DEFAULT 'in_progress' CHECK (status IN ('in_progress', 'completed', 'absent')),
...
CREATE INDEX idx_unified_attendance_user_date ON public.unified_attendance(user_id, entry_date);
CREATE INDEX idx_unified_attendance_employee_code ON public.unified_attendance(employee_code);
CREATE INDEX idx_unified_attendance_source ON public.unified_attendance(source);
CREATE INDEX idx_unified_attendance_created_at ON public.unified_attendance(created_at);

-- Create unique constraint to prevent duplicates
CREATE UNIQUE INDEX ux_unified_attendance_user_date 
...
-- Policy for system/API to view all attendance (service role)
CREATE POLICY "System can view all attendance" ON public.unified_attendance
...
-- Policy for system/API to insert attendance (service role)
CREATE POLICY "System can insert attendance" ON public.unified_attendance
...
-- Policy for system/API to update attendance (service role)
CREATE POLICY "System can update attendance" ON public.unified_attendance
...
DO $$
BEGIN
  RAISE NOTICE 'Unified Attendance Table Created Successfully!';
  RAISE NOTICE '  Table: unified_attendance';
  RAISE NOTICE '  RLS policies: Enabled';
  RAISE NOTICE '  Processing function: process_teamoffice_unified_attendance';
