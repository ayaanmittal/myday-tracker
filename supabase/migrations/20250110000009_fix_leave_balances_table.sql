-- Fix leave_balances table structure and create missing function

-- First, let's check if the leave_balances table exists and has the right structure
DO $$ 
BEGIN
    ...
    ELSE
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'allocated_days') THEN
            ALTER TABLE leave_balances ADD COLUMN allocated_days INTEGER DEFAULT 0;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'used_days') THEN
            ALTER TABLE leave_balances ADD COLUMN used_days INTEGER DEFAULT 0;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'remaining_days') THEN
            ALTER TABLE leave_balances ADD COLUMN remaining_days INTEGER DEFAULT 0;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'probation_allocated_days') THEN
            ALTER TABLE leave_balances ADD COLUMN probation_allocated_days INTEGER DEFAULT 0;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'probation_used_days') THEN
            ALTER TABLE leave_balances ADD COLUMN probation_used_days INTEGER DEFAULT 0;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'probation_remaining_days') THEN
            ALTER TABLE leave_balances ADD COLUMN probation_remaining_days INTEGER DEFAULT 0;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'is_paid') THEN
            ALTER TABLE leave_balances ADD COLUMN is_paid BOOLEAN DEFAULT true;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leave_balances' AND column_name = 'requires_approval') THEN
            ALTER TABLE leave_balances ADD COLUMN requires_approval BOOLEAN DEFAULT true;
        END IF;
    END IF;
END $$;
...
-- Create the refresh_employee_leave_balances function
...
-- Create the get_employee_leave_summary function
...
-- Create the update_leave_usage function
