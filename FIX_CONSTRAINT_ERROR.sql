-- Fix the ON CONFLICT error by adding proper constraints
-- This will resolve the constraint matching issue

-- 1. First, check what constraints exist on employee_salaries
SELECT 
  conname as constraint_name,
  contype as constraint_type,
  pg_get_constraintdef(oid) as constraint_definition
FROM pg_constraint 
WHERE conrelid = 'public.employee_salaries'::regclass;

-- 2. Add a unique constraint if it doesn't exist
-- This ensures we can use ON CONFLICT properly
ALTER TABLE public.employee_salaries 
ADD CONSTRAINT IF NOT EXISTS ux_employee_salaries_user_effective 
UNIQUE (user_id, effective_from);

-- 3. Now recreate the RPC functions without the problematic ON CONFLICT
...
-- 4. Create the get_employee_salary_summary function
...
-- 5. Create the get_employee_leaves_with_salary_deductions function
...
-- 6. Create the get_employee_salary_payment function
...
-- 8. Insert sample data without ON CONFLICT
